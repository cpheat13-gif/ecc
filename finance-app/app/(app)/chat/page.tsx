...
const SUGGESTIONS = [
  'How much does each of us contribute per month?',
  'What percentage of income goes to fixed expenses?',
  'If I increase savings by $200/month, how does it affect our vaults?',
  'Summarize our financial plan',
]
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (err.error?.includes('ANTHROPIC_API_KEY')) setConfigured(false)
      ...
      setMessages([...newMessages, { role: 'assistant', content: `Connection error: ${err}` }])
    } finally {
      setStreaming(false)
    }
  }
    ...
    <div className="max-w-3xl mx-auto flex flex-col h-[calc(100vh-8rem)]">
      ...
      {!configured && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4 text-sm text-amber-800">
          <strong>Setup required:</strong> Add your <code className="bg-amber-100 px-1 rounded">ANTHROPIC_API_KEY</code> to{' '}
          <code className="bg-amber-100 px-1 rounded">finance-app/.env.local</code> to enable the AI assistant.
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto bg-white rounded-2xl border border-gray-200 p-4 space-y-4 mb-4">
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">Ask me about your finances — I know your vault goals, income, and spending plans.</p>
