...
export default function MonthlyPage() {
  ...
  const ccLeftover = totalIncome * (ccVault.goal_amount / vaults.reduce((s, v) => s + v.goal_amount, 0)) - totalCC
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <SpendingCard
          ...
          total={totalFixed}
          totalLabel="Total Planned Expenses"
          footer={
            <Row
              label="Planned vs Goal"
              value={totalFixed - fixedVault.goal_amount}
              signed
            />
          }
        >
        ...
        <SpendingCard
          ...
          footer={
            <>
              <Row label="Total Planned" value={-totalCC} signed />
              <Row label="Leftover Money" value={ccLeftover} signed />
            </>
          }
          header={
            <div className="flex justify-between text-sm text-gray-500 mb-3">
              <span>Combined Spendable</span>
              <span className="font-semibold text-gray-800">
                {formatCurrency(totalIncome * (ccVault.goal_amount / vaults.reduce((s, v) => s + v.goal_amount, 0)))}
              </span>
            </div>
          }
        >
...
function SpendingCard({
  title, icon, cardClass, children, total, totalLabel, footer, header,
}: {
  title: string
  icon: string
  cardClass: string
  ...
  return (
    <div className={`rounded-2xl border p-5 ${cardClass}`}>
      ...
      <div className="mt-4 pt-3 border-t border-black/10 space-y-1">
        <Row label={totalLabel} value={-total} signed />
        {footer}
      </div>
    </div>
  )
