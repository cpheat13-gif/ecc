import type { Vault, MemberSettings, PaycheckDistribution, PaycheckBreakdown, CoupleSettings } from '@/types/app'
...
// CC vault goal is always the remainder after income minus all other vault goals.
// Call this after any update that touches income or non-CC vault goals.
export function withCCRebalanced(settings: CoupleSettings): CoupleSettings {
  const totalIncome = settings.members.reduce(
    (s, m) => s + m.avg_paycheck * m.paychecks_per_month, 0,
  )
  const otherTotal = settings.vaults
    .filter(v => v.id !== 'credit-card')
    .reduce((s, v) => s + v.goal_amount, 0)
  const ccGoal = Math.max(0, Math.round(totalIncome - otherTotal))
  return {
    ...settings,
    vaults: settings.vaults.map(v =>
      v.id === 'credit-card' ? { ...v, goal_amount: ccGoal } : v,
    ),
  }
}
