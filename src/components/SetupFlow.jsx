    ...
    <div className={`rounded-2xl border p-4 transition-colors ${
        ...
        {MEAL_TYPES.map(meal => (
          <button key={meal} onClick={() => toggleMeal(meal)}
            className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${
              config.meals.includes(meal) ? 'bg-emerald-500 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
            }`}>
      ...
      {config.meals.length > 0 && (
        <div className="space-y-2 pt-3 border-t border-slate-700">
          {[{ key: 'connorTraining', label: 'Connor' }, { key: 'isaTraining', label: 'Isa' }].map(({ key, label }) => (
            <div key={key} className="flex items-center justify-between">
              <span className="text-xs text-slate-400">{label}</span>
              <div className="flex gap-1.5">
                {['Training', 'Rest'].map(mode => (
                  <button key={mode} onClick={() => onChange({ ...config, [key]: mode === 'Training' })}
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                      (config[key] ? 'Training' : 'Rest') === mode
                        ? mode === 'Training' ? 'bg-sky-500 text-white' : 'bg-slate-600 text-slate-200'
...
export default function SetupFlow() {
  ...
  const updateDay = (day, dayConfig) => setConfig(prev => ({ ...prev, [day]: dayConfig }));
  const handleBuild = () => dispatch({ type: 'COMPLETE_SETUP', weekConfig: config });
  const handlePreset = (preset) => {
    const next = { ...config };
    DAYS.forEach(day => {
      const isWeekend = day === 'saturday' || day === 'sunday';
      if (preset === 'weekday-dinner') next[day] = { meals: isWeekend ? [] : ['dinner'], connorTraining: !isWeekend, isaTraining: false };
      else if (preset === 'full') next[day] = { meals: ['breakfast', 'lunch', 'dinner', 'snack'], connorTraining: !isWeekend, isaTraining: !isWeekend };
      else if (preset === 'clear') next[day] = { meals: [], connorTraining: false, isaTraining: false };
    });
    setConfig(next);
  };
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
      <div className="px-4 pt-12 pb-6">
        ...
        <p className="text-slate-400 text-sm mt-1">Select meals and training days for Connor &amp; Isa.</p>
        <div className="flex gap-2 mt-4 overflow-x-auto pb-1">
          {[{ id: 'weekday-dinner', label: 'Weekday dinners' }, { id: 'full', label: 'Full week' }, { id: 'clear', label: 'Clear all' }].map(p => (
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-3 pb-32">
        {DAYS.map(day => <DayCard key={day} day={day} config={config[day]} onChange={(c) => updateDay(day, c)} />)}
      </div>
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-slate-900 via-slate-900/95 to-transparent pb-8">
        <button onClick={handleBuild} disabled={totalMeals === 0}
          ...
          {totalMeals === 0 ? 'Select at least one meal' : `Build My Plan · ${totalMeals} meal${totalMeals !== 1 ? 's' : ''}`}
        </button>
        <p className="text-center text-xs text-slate-500 mt-2">Mon-Fri dinners pre-loaded with 5 seed recipes</p>
      </div>
    </div>
  );
}
